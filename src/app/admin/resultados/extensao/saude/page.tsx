...
export default function Home() {
	...
	const filtered = avaliacoes?.filter((item) => item.areaTrabalho === "Saúde");
	...
	return (
		<main className="bg-white flex flex-col items-center min-h-screen space-y-10">
			...
			{filtered && filtered.length > 0 ? (
					...
					<div className="w-full grid grid-cols-1 md:grid-cols-4 md:space-x-4">
					...
					{filtered?.map((item, index, array) => (
						<>
							<div
								className="w-full grid grid-cols-1 md:grid-cols-4 md:space-x-4"
								key={item.trabalhoId}
							>
